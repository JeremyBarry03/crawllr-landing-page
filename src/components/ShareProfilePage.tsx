...
const buildDeepLink = (userId: string) =>
  `crawllr://user/${encodeURIComponent(userId)}`;

export const ShareProfilePage = ({ userId }: ShareProfilePageProps) => {
  const deepLink = useMemo(() => buildDeepLink(userId), [userId]);
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12 text-white">
        <div className="w-full max-w-md rounded-[24px] border border-white/10 bg-white/10 p-8 text-center shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur">
          ...
          <p className="mt-3 text-sm text-white/80">
            We&apos;re opening this profile in the app. If nothing happens,
            tap below.
          </p>
